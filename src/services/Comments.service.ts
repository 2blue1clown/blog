...
export const CommentsService = {
  async get(blogName: string) {
    ...
    try {
      const res = await fetch(`/api/comments/${blogName}`, {
        method: "GET",
        mode: "cors",
        headers: {
          Authorization: "Bearer <JWT>", //window.localStorage.getItem("jwt"),
      ...
      const comments: BlogComment[] = responseJson.map(
        (commentDTO) => new BlogComment(commentDTO)
      );
      return comments;
    } catch (e) {
      console.error(e);
